...
class BeneficentialDiaryBook extends BaseController
{
    ...
    function _beneficentialdiarybook_output($output = null)
    {
        $this->load->view('beneficentialdiarybook_view.php',$output);
    }

    function index()
    {
        $this->_beneficentialdiarybook_output((object)array('output' => '' , 'js_files' => array() , 'css_files' => array(), 'base' => $this->config->item('base_url'), 'css' => 'style.css', 'entries' => array()));
    }

    function renderBeneficentialDiaryBook($array, $start_date, $end_date)
    {
        /// monta tela
        $ret = "";

        $ret .= "<a href='".$this->config->item("base_url")."index.php/beneficentialdiarybook/printBeneficentialDiaryBook/".$start_date."/".$end_date."' class=' crud-action' title='Imprimir Caixa' target='_blank'><img src='".$this->config->item("base_url")."application/views/common/images/print_icon.png' alt='Imprimir Caixa' /></a>";
...
//        $saldoAnterior = $value_receita_last_month - $value_despesa_last_month;
        $saldoAnterior = $this->retrieveBeneficentialBalanceUntilDate($date_select_end_last_month);
...
//var_duMP($saldoAtual);var_dump($saldoAnterior);die();
    ...
    function retrieveBeneficentialEntriesForPeriod($date_select_start, $date_select_end)
    {
        $array = array();
        try{
            $this->db->select("beneficential_entries.id, beneficential_entries.lodge_id, beneficential_entries.user_id, users.name, beneficential_entries.date, beneficential_entries.description, beneficential_entries.value")
                ->from("beneficential_entries")
                ->join('users', 'beneficential_entries.user_id = users.id')
                ->where("beneficential_entries.date >= '".$date_select_start."' and beneficential_entries.date <= '".$date_select_end."' and beneficential_entries.lodge_id='".$_SESSION['lodge_id_as_admin']."'")
                ->order_by("beneficential_entries.date", "asc");
            $db = $this->db->get();
    ...
    function retrieveBeneficentialBalanceUntilDate($date_select_end)
    {
        $array = array();
        try{
            $this->db->select("SUM(value) AS balance")
                ->from("beneficential_entries")
                ->where("date <= '".$date_select_end."' and beneficential_entries.lodge_id='".$_SESSION['lodge_id_as_admin']."'");
            $db = $this->db->get();
    ...
    function diarybook_beneficential_report()
    {
        ...
        $array = $this->retrieveBeneficentialEntriesForPeriod($date_select_start, $date_select_end);

        $output['base'] = $this->config->item('base_url');
        $output['css'] = 'style.css';
        $output['output'] = $this->renderBeneficentialDiaryBook($array, $date_select_start, $date_select_end);

        $this->_beneficentialdiarybook_output($output);
    }
    ...
    */
    function printBeneficentialDiaryBook($start_date, $end_date)
    {
        $array = $this->retrieveBeneficentialEntriesForPeriod($start_date, $end_date);
